...
select 
	total.day as Day, 
	isnull(round(cast(canceled.cnt as decimal(6,2))/cast (total.cnt as decimal(6,2)),2),0) as 'Cancellation Rate'
    from (
    	select 
    		request_at as Day,
...
group by 
total.day, 
isnull(round(cast(canceled.cnt as decimal(6,2))/cast (total.cnt as decimal(6,2)),2),0)